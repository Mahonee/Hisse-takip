...
components.html(
    ...
    <script>
        ...
        // --- HİÇBİR RENDER / BEYAZ EKRAN OLUŞTURMADAN 5 SANİYEDE BİR GÜNCELLEME MEKANİZMASI ---
        setInterval(() => {
            fetch(window.parent.location.href)
                ...
                .then(html => {
                    ...
                    // Streamlit ana içerik taşıyıcısını arka planda günceller
                    const yeniIcerik = doc.querySelector('.main .block-container');
                    const mevcutIcerik = window.parent.document.querySelector('.main .block-container');
                    
                    if (yeniIcerik && mevcutIcerik) {
                        mevcutIcerik.innerHTML = yeniIcerik.innerHTML;
                    }
                })
                .catch(error => console.error('Arka plan güncelleme hatası:', error));
